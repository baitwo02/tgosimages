#!/usr/bin/env bash
#
# Inject lspci (pciutils) into an existing Alpine ext4 rootfs image without
...
IMAGE_PATH="${IMAGE_PATH:-${ROOT_DIR}/IMAGES/rootfs/rootfs-aarch64-alpine-ivc-message-v1.img}"
WORK_DIR="${WORK_DIR:-${BUILD_DIR}/inject-lspci}"

usage() {
    printf 'Inject lspci (pciutils) into an Alpine ext4 rootfs image\n'
    printf '\n'
    printf 'Usage:\n'
    printf '  scripts/tools/inject-lspci.sh [options]\n'
    printf '\n'
    printf '[options]:\n'
    printf '  --image <path>    Target ext4 image (default: IMAGES/rootfs/rootfs-aarch64-alpine-ivc-message-v1.img)\n'
