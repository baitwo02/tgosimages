...
SDK 的分层结构、共享内存布局、消息格式和生命周期说明见 [SDK_DESIGN_CN.md](SDK_DESIGN_CN.md)。
...
在 SDK 根目录执行一条命令即可构建 Linux C++ 示例程序：
...
```text
build/linux/ivshmem-linux-demo
```

默认使用本机 `cc` 和 `c++`，因此生成的是本机可执行文件。需要交叉编译时直接覆盖编译器即可：

```sh
make linux CC=aarch64-linux-musl-cc CXX=aarch64-linux-musl-c++
```

Zephyr 示例不是独立用户态程序。`zephyr/src/main.cpp` 依赖 Zephyr 内核 API，会和 Zephyr 内核一起链接生成客户机镜像，不能单独用本机 `c++` 编译成普通可执行文件。
