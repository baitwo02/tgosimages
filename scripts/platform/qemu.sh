...
LINUX_SRC_DIR="${LINUX_SRC_DIR:-${BUILD_DIR}/qemu_linux}"
...
usage() {
    ...
    printf '  ivc-image                         Build the dedicated AArch64 IVC Message V1 rootfs\n'
    ...
    printf '  --base-rootfs-archive <path>      Official v0.0.12 archive for ivc-image\n'
    printf '\n'
    ...
    printf '  * IVC artifacts are packaged only by the dedicated ivc-image target.\n'
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    case "${cmd}" in
        ...
        aarch64|riscv64|x86_64|loongarch64)
            ...
            OS="$1"
            shift 1 || true
            if [[ "${OS}" == "ivc-image" ]]; then
                [[ "${ARCH}" == "aarch64" ]] \
                    || die "The dedicated IVC image is supported only for qemu aarch64"
                exec "${ROOT_DIR}/scripts/platform/qemu-ivc-image.sh" "$@"
            fi
            qemu_rootfs_parse_args "$@"
            ...
            case "${OS}" in
                ...
                clean)
                    ...
                    ;;
                *)
                    die "Unknown os: ${OS} (supported: linux, ivc-image, arceos, zephyr, freertos, all)"
                    ;;
            esac
            ;;
        all)
